...
namespace IMS.Controllers
{
    ...
    public class HomeController : Controller
    {
        ...
        private static string WebAPIURL = "https://localhost:44390/";
        public async Task<ActionResult> Index()
        {
            ...
            //    List<StudentListForAdmissionDetailsByInstituteId_Result> allStudents = AdmissionBL.GetStudentAdmissionByInstituteId(Convert.ToInt32(model.InstituteId));
        ...
        {
            var _context = new TECHNOSOFTSERVICESEntities();
            ...
            TempData.Keep();
            List<CoursewiseStudentCount_PieChart_Result> results = CourseBL.GetAllCoursesStudentCount(model.InstituteId, DateTime.Now.Year);
            results.ToList().ForEach(re => xvalue.Add(re.COURSENAME));
            results.ToList().ForEach(re => yvalue.Add(re.STUDENTCOUNT));

            new Chart(width: 600, height: 400, theme: ChartTheme.Green)
                .AddTitle("Coursewise Student Count")
        ...
        [HttpPost]
        public ActionResult GetPieChart(string year)
        {
            User model = TempData["UserObject"] as User;
            TempData.Keep();
            List<DataPoint> dataPoint = new List<DataPoint>();
            List<CoursewiseStudentCount_PieChart_Result> results = CourseBL.GetAllCoursesStudentCount(model.InstituteId, Convert.ToInt32(year));
            foreach (var item in results)
            {
                dataPoint.Add(new DataPoint(item.COURSENAME, Convert.ToDouble(item.STUDENTCOUNT)));
            }
